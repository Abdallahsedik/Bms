    ...
    stages {
        ...
        stage('3. Static Analysis (MISRA-C)') {
            steps {
                ...
                bat 'cppcheck --enable=all --xml --xml-version=2 mcu/ 2> cppcheck-result.xml || exit 0'            post {
                always {
        ...
        stage('4. Native Unit Tests (Host)') {
            steps {
                ...
                // We will comment this out just for this run since we haven't written the tests yet!
                // bat 'make test_host' 
            }
        }
